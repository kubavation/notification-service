...
internal class NotificationServiceTest {
    ...
    @Test
    fun markNotificationsAsDeleted_shouldChangeStatusToDeleted() {
        val notifications = listOf(
                NotificationDTO(TenantId("1"), "test1", "test1", listOf(NotificationType.APP))
                        .withId("1"),
                NotificationDTO(TenantId("2"), "test2", "test2", listOf(NotificationType.APP))
        ...
        notificationService.markAsDeleted(notifications)

        entities.forEach {
            assertEquals(NotificationStatus.DELETED, it.status)
        }
    }
}